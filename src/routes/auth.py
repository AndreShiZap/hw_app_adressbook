...
async def request_password_reset(email: EmailStr, request: Request, db: AsyncSession = Depends(get_db)):
    user = await repositories_users.get_user_by_email(email, db)
    if user is None:
...
async def reset_password(token: str, new_password: str, db: AsyncSession = Depends(get_db)):
    ...
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    updated_user = await repositories_users.update_user_password(email, new_password, db)
    auth_service.cache.delete(email)
    return {"message": f"Password updated successfully for user {updated_user.username}"}
